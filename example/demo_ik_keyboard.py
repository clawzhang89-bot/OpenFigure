...
class KeyboardIKDemo:
    def __init__(self, pos_step: float = 0.005, rot_step: float = 0.03, freq: float = 50.0):
    ...
    def step(self):
        ...
        if self._frame_count % int(self.freq) == 0:
            ...
            print(f"[{hand}] pos=[{pos[0]:.3f},{pos[1]:.3f},{pos[2]:.3f}] " f"rpy=[{np.degrees(eul[0]):.1f},{np.degrees(eul[1]):.1f},{np.degrees(eul[2]):.1f}]°")
        return mit_cmd
